...
public class ReportProfitLossAdapter extends RecyclerView.Adapter<ReportProfitLossAdapter.ViewHolder> implements Filterable {
    ...
    public ReportProfitLossAdapter(Context context, ArrayList<ItemModel2> list) {
        this.list = list;
    ...
    @Override
    public ReportProfitLossAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.list_report_row_item, viewGroup, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(final ReportProfitLossAdapter.ViewHolder viewHolder, final int i) {
        if (i % 2 == 0) {
...
//        boolean isExpanded = list.get(i).isExpanded();
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (ItemModel2 item : exampleListFull) {
                    ...
                    else if (item.getRemarks().toLowerCase().contains(filterPattern)) {
                        filteredList.add(item);
                    }
